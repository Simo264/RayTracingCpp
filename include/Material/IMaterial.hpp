...
{
public:
	IMaterial(glm::vec3 color_scale = glm::vec3(0.f),
						std::shared_ptr<Texture2D> color_texture = nullptr,
						float roughness_scale = 0.f,
						std::shared_ptr<Texture2D> roughness_texture = nullptr,
						glm::vec3 emission_scale = glm::vec3(0.f),
					  std::shared_ptr<Texture2D> emission_texture = nullptr) :
		emission_scale{ emission_scale },
		color_scale{ glm::clamp(color_scale, 0.f, 1.f) },
		roughness_scale{ glm::clamp(roughness_scale, 0.f, 1.f) },
	...
	virtual glm::vec3 emitted(const HitRecord& hit) const = 0;
};
